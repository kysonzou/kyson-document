思考：一个当键入网址后，到网页显示，期间到底发生了什么呢？

**一个数据包在网络中的心路历程**

OSI模型

![OSI模型](../assets/network-数据包-OSI模型.png)

#### 正文

接下来以下图较简单的网络拓扑模型作为例子，探究探究其间发生了什么？

![network-简单的网路模型](../assets/network-数据包-简单的网路模型.jpeg)
...
让我们看看一条长长的 URL 里的各个元素代表什么，见下图：



![network-url解析](../assets/network-数据包-url解析.jpeg)
...
![network-HTTP的消息个格式](../assets/network-数据包-HTTP的消息个格式.jpeg)



一个孤单 HTTP 数据包表示：“我这么一个小小的数据包，没亲没友，直接发到浩瀚的网络，谁会知道我呢？谁能载我一层（程）呢？谁能保护我呢？我的目的地在哪呢？” 。充满各种疑问的它，没有停滞不前，依然踏上了征途！
...
毕竟域名是外国人发明的，所以思维和中国人相反，比如说一个城市地点的时候，外国喜欢从小到大的方式顺序说起（如 XX 街道 XX 区 XX 市 XX 省），而中国则喜欢从大到小的顺序（如 XX 省 XX 市 XX 区 XX 街道）。
...
![network-根域](../assets/network-数据包-根域.png)
...
![network-域名解析](../assets/network-数据包-域名解析.jpeg)
...
![network-协议栈](../assets/network-数据包-协议栈.jpeg)



应用程序（浏览器）通过调用 Socket 库，来委托协议栈工作。协议栈的上半部分有两块，分别是负责收发数据的 TCP 和 UDP 协议，它们俩会接受应用层的委托执行收发数据的操作。
...
我们先看看 TCP 报文头部的格式：![TCP数据段](../assets/network-数据包-TCP数据段.png) 包头格式

首先，源端口号和目标端口号是必不可少的，如果没有这两个端口号，数据就不知道应该发给哪个应用。

接下来是包的序号，这个是为了解决包乱序的问题。
...
还有一个重要的就是窗口大小。TCP 要做流量控制，通信双方各声明一个窗口（缓存大小），标识自己当前能够处理的能力，别发送的太快，撑死我，也别发的太慢，饿死我。
...
![network- TCP三次握手](../assets/network-数据包-TCP三次握手.png)
...
[![TCP 连接状态查看](../assets/network-数据包-TCP连接状态查看.png)
...
[![MTU 与 MSS](../assets/network-数据包-MTU与MSS.png)
...
![network-数据包分割](../assets/network-数据包-数据包分割.jpeg)
...
在双方建立了连接后，TCP 报文中的数据部分用来存放 HTTP 头部 + 数据，组装好 TCP 报文之后，就需交给下面的网络层处理。

至此，网络包的报文如下图所示：

[![TCP 层报文](../assets/network-数据包-TCP层报文.jpeg)](https://ccie.lol/wp-content/uploads/2020/04/the-journey-of-a-packet-in-the-network-12.jpeg)TCP 层报文

此时，遇上了 TCP 的数据包激动表示：“太好了，碰到了可靠传输的 TCP 传输，它给我加上 TCP 头部，我不在孤单了，安全感十足啊！有大佬可以保护我的可靠送达！但我应该往哪走呢？”
...
![IP数据包](../assets/network-数据包-IP数据包.png)IP 包头格式
...
![network-路由表](../assets/network-数据包-路由表.png)

举个例子，根据上面的路由表，我们假设 Web 服务器的目标地址是 192.168.10.200 。

[![路由规则判断](../assets/network-数据包-路由规则判断.jpeg)](https://ccie.lol/wp-content/uploads/2020/04/the-journey-of-a-packet-in-the-network-15.jpeg)路由规则判断

1. 首先先和第一条条目的子网掩码（ Genmask ）进行与运算，得到结果为 192.168.10.0 ，但是第一个条目的 Destination 是 192.168.3.0 ，两者不一致所以匹配失败。
...
![network-IP层报文](../assets/network-数据包-IP层报文.jpeg)
...
![network-MAC包头格式](../assets/network-数据包-MAC包头格式.png)

在 MAC 包头里需要发送方源 MAC 地址和接收方目标 MAC 地址，用于两点之间的传输。
...
![network-ARP广播](../assets/network-数据包-ARP广播.png)
...
放心，在这之后，操作系统会把本次查询结果放到一块叫做 ARP 缓存的内存空间留着以后用，不过缓存的时间就几分钟。
...
[![ARP 缓存内容](../assets/network-数据包-ARP缓存内容.png)](https://ccie.lol/wp-content/uploads/2020/04/the-journey-of-a-packet-in-the-network-19.png)ARP 缓存内容
...
[![MAC 层报文](../assets/network-数据包-Mac层报文.jpeg)
...
[![物理层数据包](../assets/network-数据包-物理层数据包.png)](https://ccie.lol/wp-content/uploads/2020/04/the-journey-of-a-packet-in-the-network-21.png)物理层数据包
...
![network-交换机的MAC地址表](../assets/network-数据包-交换机的MAC地址表.jpeg)
...
地址表中找不到指定的 MAC 地址。这可能是因为具有该地址的设备还没有向交换机发送过包，或者这个设备一段时间没有工作导致地址从地址表中删除了。
...
当转发包时，首先路由器端口会接收发给自己的以太网包，然后通过路由表查询转发目标，再由相应的端口作为发送方将以太网包发送出去。
...
![network-路由器转发](../assets/network-数据包-路由器转发.jpeg)
...
- 如果网关是一个 IP 地址，则这个 IP 地址就是我们要转发到的目标地址，还未抵达终点，还需继续路由器转发。
- 如果网关为空，则 IP 头部中的接收方 IP 地址就是要转发到的目标地址，也就是终于找到 IP 包头里的目标地址了，说明已抵达终点。
...
数据包通过多个路由器道友的帮助，在网络世界途径了很多路程，最终抵达了目的地的城门！城门值守的路由器，发现了这个小兄弟数据包原来是找城内的人，于是它就将数据包送进了城内，再经由城内的交换机帮助下，最终转发到了目的地。数据包感慨万千的说道：“多谢这一路上，各路大侠的相助！”
...
[![扒皮模型](../assets/network-数据包-扒皮模型.jpeg)
...
[一个数据包在网络中的心路历程](https://xiaolincoding.com/network/1_base/what_happen_url.html#两点传输-mac)